...
class PowerOutage:
    def estimateTimeOut(self, fromJunction, toJunction, ductLength):
        ...
        self.ductLength = ductLength
        self.not_terminate_symbol_list = []
        cost = 0
        self.pieces = []
        self.longest_pices = []
        self.get_route(0)
        ...
        self.not_terminate_symbol_list = list(set(self.not_terminate_symbol_list))
        self.not_terminate_symbol_list.remove(0)

        for (f,t,c) in self.pieces:
            if t in self.not_terminate_symbol_list or t == max(self.toJunction):
                cost = cost+c
            else:
                cost = cost+c*2
        return cost
    ...
    def get_route(self,base_number):
        for i in range(len(self.fromJunction)):
            if self.fromJunction[i] == base_number:
                next_num = self.toJunction[i]
                cost = self.ductLength[i]
                self.pieces.append( (base_number,next_num ,cost) )
                self.not_terminate_symbol_list.append( base_number )
                if next_num in self.fromJunction:
                    self.get_route(next_num)
